...
var gameOver = function gameOver() {
  return $('.turn-message').text('The game has finished. Select "New Game".');
};
...
var gameEnd = function gameEnd(boolean) {
  if (boolean) {
    gameOver();
...
var winCheck = function winCheck(token) {
...
var playerAvatar = function playerAvatar(turnCount) {
...
var turnMessage = function turnMessage() {
  if (isEven(turnCount)) {
...
var apiCheck = 0;
var updateCell = function updateCell(cellIndex) {
  ...
  var turnValue = playerAvatar(turnCount);
  if (store.user) {
    var data = {
      game: {
        id: store.game.id,
...
var gameAction = function gameAction(cellIndex) {
  if (over) {
    gameOver();
  } else {
...
var onCellSelect = function onCellSelect(event) {
  var cellData = this.dataset;
  var cellIndex = cellData.cellIndex;
...
var onSignIn = function onSignIn(event) {
  var data = getFormFields(this);
  event.preventDefault();
  api.signIn(data).then(ui.signInSuccess).catch(ui.signInFailure).then(api.createGame).then(ui.createGameSuccess).then(api.getStats).then(ui.getStatsSuccess).then(newGame());
};
...
var onSignOut = function onSignOut(event) {
  event.preventDefault();
  api.signOut().then(ui.signOutSuccess).then(newGame());
...
var users = function users(email) {
  var ampersand = email.indexOf('@');
  var userName = email.substring(0, ampersand);
  return userName;
};
...
var getStatsSuccess = function getStatsSuccess(data) {
  ...
  var tokenX = winCheck('x');
  var tokenO = winCheck('o');
  var tie = store.games.length - (tokenX + tokenO);
